...
int main(int argc, char *argv[])
{
    ...
    if(world_rank==0)
    {
        ...
        for(int j=1;j<world_size;j++)
        {
            ...
            temp_end_row=(start_row+(per_processor_rows-1));
            // matrix_A_data[ptr_vec[temp_start_row]] to matrix_A_data[ptr_vec[temp_end_row+1]-1]
            // temp_temp=
            MPI_Send(&ptr_vec[temp_start_row],1,MPI_LONG_LONG_INT,j,(j*3),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Send(&ptr_vec[temp_end_row+1],1,MPI_LONG_LONG_INT,j,(j*3)+1,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Send(&matrix_A_data[ptr_vec[temp_start_row]],(ptr_vec[temp_end_row+1]-ptr_vec[temp_start_row]),MPI_LONG_LONG_INT,j,(j*3)+2,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Send(&matrix_A_row[ptr_vec[temp_start_row]],(ptr_vec[temp_end_row+1]-ptr_vec[temp_start_row]),MPI_LONG_LONG_INT,j,(j*3)+3,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Send(&matrix_A_col[ptr_vec[temp_start_row]],(ptr_vec[temp_end_row+1]-ptr_vec[temp_start_row]),MPI_LONG_LONG_INT,j,(j*3)+4,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    else
    {
        ...
        MPI_Recv(&temp_start_ptr,1,MPI_LONG_LONG_INT,0,(j*3),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(&temp_end_ptr,1,MPI_LONG_LONG_INT,0,(j*3+1),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        MPI_Recv(&matrix_A_data[0],((temp_end_ptr)-(temp_start_ptr)),MPI_LONG_LONG_INT,0,(j*3+2),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(&matrix_A_row[0],((temp_end_ptr)-(temp_start_ptr)),MPI_LONG_LONG_INT,0,(j*3+3),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(&matrix_A_col[0],((temp_end_ptr)-(temp_start_ptr)),MPI_LONG_LONG_INT,0,(j*3+4),MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ptr_vec.resize(dimension+1);
        MPI_Bcast(&ptr_vec[0], (dimension+1), MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD);
    }
    ...
    if(world_rank==0)
    {
        dimension_size=(one_start_index*sizeof(long long int));
    }
